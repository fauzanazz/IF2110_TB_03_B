...
void ConvertTimeTtoDATETIME(time_t current_time, DATETIME *convertedTime) {
    struct tm *local_time = localtime(&current_time); // Convert time_t to struct tm

    // Populate the DATETIME structure
    convertedTime->DD = local_time->tm_mday;
    convertedTime->MM = local_time->tm_mon + 1; // tm_mon is 0-indexed
    convertedTime->YYYY = local_time->tm_year + 1900; // tm_year is years since 1900
    convertedTime->T.HH = local_time->tm_hour;
    convertedTime->T.MM = local_time->tm_min;
    convertedTime->T.SS = local_time->tm_sec;
}
